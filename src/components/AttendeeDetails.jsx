...
const AttendeeDetails = () => {
    ...
    const uploadToCloudinary = async (file) => {
        ...
        try {
            const res = await fetch("https://api.cloudinary.com/v1_1/drkknoddw/image/upload", {
                method: "POST",
                body: formData,
            });
        ...
        <div className='sm:border border-[#0E464F] my-6 sm:p-6 rounded-3xl'>
            <div className="border border-[#0E464F] rounded-3xl p-6 mb-6 text-white bg-[#052228]">
                ...
                <div className="w-full h-36 flex items-center justify-center bg-[#041b20] mb-5">
                    ...
                    >
                        {avatar ? (
                            <img src={avatar} alt="Avatar Preview" className="w-full h-full object-cover border" />
                        ) : (
                            <>
            ...
            <div className="text-white mb-6">
                ...
                <input type="text" className='w-full outline-none border border-[#0E464F] bg-transparent px-4 py-2 rounded-xl text-base focus:border-2' required />
            </div>
            ...
            <div className="text-white mb-6">
                ...
                <div className="w-full flex items-center gap-2 outline-none border border-[#0E464F] px-3 rounded-xl active:border-2 focus:border-2">
                    ...
                    <input type="email" className='w-full h-full outline-none bg-transparent px-2 py-3 text-base' required />
                </div>
            </div>

            <div className="text-white mb-6">
                <h3 className='text-[16px] mb-4'>Special request?</h3>
                <textarea name="" id="" className='p-4 h-36 w-full outline-none bg-transparent border border-[#0E464F] rounded-xl text-base focus:border-2'></textarea>
            </div>

            <div className="flex flex-col sm:flex-row gap-2">
                <button onClick={prevStep} className='order-2 sm:order-1 flex-1 border border-[#24A0B5] rounded-xl py-3 text-base sm:text-sm md:text-lg text-[#24A0B5] hover:bg-[#24A0B5] hover:text-white transition-all ease-in-out duration-500'>Back</button>
                <button onClick={nextStep} className='order-1 sm:order-2 flex-1 border border-[#24A0B5] rounded-xl py-3 text-base sm:text-sm md:text-lg text-[#24A0B5] hover:bg-[#24A0B5] hover:text-white transition-all ease-in-out duration-500'>Get My Free Ticket</button>
            </div>
        </div>
    )
}
